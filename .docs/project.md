...
- **Audio recovery**: device disconnect → drop exclusive output, restore device state, install shared on system default, push a `Recovered` event for a notification toast
- **User-visible errors**: surfaced via `gpui-component::notification::Notification` and via `OutputEvent` drained on each render of `AudioSettings`
