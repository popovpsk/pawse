use crate::i18n::Strings;
use crate::lang;

pub static NL: Strings = lang! {
    plural: OneOther,
    reveal_folder: ("Tonen in Finder", "Tonen in Verkenner", "Map openen"),
    play: "Afspelen",
    pause: "Pauzeren",
    next: "Volgende",
    previous: "Vorige",
    shuffle: "Willekeurig",
    repeat_mode: "Herhaalmodus",
    volume: "Volume",
    mute: "Dempen",
    unmute: "Dempen opheffen",
    queue: "Wachtrij",
    remove_from_queue: "Uit wachtrij verwijderen",
    queue_is_empty: "De wachtrij is leeg.",
    clear_queue: "Wachtrij leegmaken",

    add_to_playlist: "Aan afspeellijst toevoegen",
    remove_from_playlist: "Uit afspeellijst verwijderen",
    add_to_queue: "Aan wachtrij toevoegen",
    add_album_to_queue: "Album aan wachtrij toevoegen",
    add_artist_tracks_to_queue: "Nummers van artiest aan wachtrij toevoegen",
    full_albums_tooltip: "Sommige albums tonen alleen de nummers waaraan deze artiest meewerkte. Schakel dit in om die albums volledig te tonen, inclusief nummers van andere artiesten.",
    replace_queue: "Wachtrij vervangen",
    replace_queue_confirm_title: "Wachtrij vervangen?",
    replace_queue_confirm_message: "Je hebt de huidige wachtrij bewerkt. Dit nummer afspelen vervangt de wachtrij. Je kunt het ook aan de wachtrij toevoegen.",
    add_to_liked: "Toevoegen aan favorieten",
    remove_from_liked: "Uit favorieten verwijderen",

    settings: "Instellingen",
    search_placeholder: "Zoeken...",
    tab_albums: "Albums",
    tab_artists: "Artiesten",
    tab_liked: "Favorieten",
    tab_playlists: "Afspeellijsten",
    cover_mode: "Hoesweergave",
    full_screen: "Volledig scherm",
    all_tracks: "Alle nummers",
    no_metadata: "Geen metadata",

    scanning: "Scannen...",
    library_updating: "Bibliotheek bijwerken…",
    library_updated: "Bibliotheek bijgewerkt",
    library_up_to_date: "Bibliotheek is up-to-date",
    library_update_failed: "Bijwerken van bibliotheek mislukt",
    library_folder_unavailable_t: "Map niet beschikbaar: {}. De nummers blijven verborgen tot de map terug is.",
    playback_failed_title: "Afspelen mislukt",
    local_folders: "Lokale mappen",
    server_list: "Servers",
    subsonic_servers_desc: "Navidrome, Gonic, Airsonic en andere Subsonic-compatibele servers. Nummers die ook in lokale mappen staan, verschijnen één keer.",
    jellyfin_servers_desc: "Jellyfin-servers. Eén keer inloggen: Pawse bewaart een toegangstoken, niet je wachtwoord. Nummers die je ook lokaal hebt, verschijnen één keer.",
    server_url: "Serveradres",
    server_username: "Gebruikersnaam",
    server_password: "Wachtwoord",
    server_connect: "Verbinden",
    server_sync: "Synchroniseren",
    server_import_favorites: "Favorieten importeren",
    network_cache: "Netwerkcache",
    network_cache_desc: "Muziek van het netwerk blijft op schijf: een al gespeeld nummer start meteen, zonder downloaden, en speelt zelfs offline.",
    cache_fill: "Opslaan in cache",
    cache_fill_progress_t: "{} van {} opgeslagen · klik om te stoppen",
    cache_fill_too_big_title: "Niet alles past in de cache",
    cache_fill_too_big_t: "Er is {} nodig, maar de cachelimiet is {}. Alleen de eerste {} van {} bestanden worden opgeslagen.",
    cache_fill_part: "Opslaan wat past",
    cache_used: "In gebruik",
    cache_clear: "Wissen",
    cache_limit: "Limiet",
    cache_limit_desc: "Boven de limiet worden de oudste nummers verwijderd.",
    cache_unlimited: "Onbeperkt",
    decimal_separator: ",",
    size_mb_t: "{} MB",
    size_gb_t: "{} GB",
    server_auth_failed: "Onjuiste gebruikersnaam of wachtwoord",
    server_unreachable_t: "Server onbereikbaar: {}",
    server_fill_fields: "Vul adres en gebruikersnaam in",
    no_servers_added: "Geen servers toegevoegd",
    torrents: "Torrents",
    torrent_list: "Bronnen",
    torrents_desc: "Albums gedeeld via BitTorrent. Pawse downloadt voor de bibliotheek alleen tags en hoezen, en haalt een nummer op als je het afspeelt.",
    torrent_magnet: "Magnetlink",
    torrent_add: "Toevoegen",
    torrent_choose_file: ".torrent-bestand kiezen…",
    torrent_invalid: "Geen magnetlink of .torrent-bestand",
    no_torrents_added: "Geen torrents toegevoegd",
    torrent_upload: "Uploaden",
    torrent_upload_desc: "Delen versnelt het downloaden. Pawse deelt alleen terwijl het van een torrent downloadt, en 15 minuten daarna.",
    torrent_upload_while_active: "Tijdens downloaden",
    torrent_upload_limited: "Tot 128 KB/s",
    torrent_upload_off: "Uit",
    torrent_peers_t: "Peers: {}/{}",
    server_online: "Online",
    server_offline: "Niet beschikbaar",
    source_syncing: "Synchroniseren…",
    remove_server_confirm_title: "Server verwijderen?",
    source_online: "Online",
    source_offline: "Niet beschikbaar",
    source_scanning: "Scannen…",
    unavailable_count_t: "Niet beschikbaar: {}",
    no_music_sources: "Nog geen muziek toegevoegd.",
    open_library_settings: "Bibliotheekinstellingen",
    no_albums_found: "Geen albums gevonden.",
    no_albums_match: "Geen albums komen overeen met je zoekopdracht.",
    no_artists_found: "Geen artiesten gevonden.",
    no_artists_match: "Geen artiesten komen overeen met je zoekopdracht.",
    no_tracks_for_album: "Geen nummers gevonden voor dit album.",
    no_tracks_match: "Geen nummers komen overeen met je zoekopdracht.",
    no_tracks_for_artist: "Geen nummers voor deze artiest.",
    no_liked_tracks: "Nog geen favorieten. Beweeg over een nummer en tik op het hart.",
    no_liked_match: "Geen favorieten komen overeen met je zoekopdracht.",
    playlist_is_empty: "De afspeellijst is leeg. Voeg nummers toe met de +-knop in elke rij.",
    unknown: "Onbekend",

    filter_playlists: "Afspeellijsten filteren",
    playlist_name: "Naam afspeellijst",
    name_new_playlist: "Nieuwe afspeellijst een naam geven",
    create: "Aanmaken",
    cancel: "Annuleren",
    no_playlists_yet: "Nog geen afspeellijsten.",
    no_playlists_match: "Geen afspeellijsten komen overeen met je zoekopdracht.",
    create_new_playlist: "Nieuwe afspeellijst aanmaken…",
    new_playlist_name: "Naam nieuwe afspeellijst",
    new_playlist: "+ Nieuwe afspeellijst",
    delete: "Verwijderen",

    settings_interface: "Interface",
    settings_general: "Algemeen",
    settings_library: "Bibliotheek",
    theme: "Thema",
    theme_desc: "Kleurenschema van de applicatie",
    dynamic_theme: "Thema volgt de hoes",
    dynamic_theme_desc: "Past de interfacekleuren aan de hoes van het huidige album aan: licht of donker schema en accentkleur. Hoezen zonder uitgesproken kleur behouden je gekozen thema.",
    system: "Systeem",
    language: "Taal",
    language_desc: "Taal van de gebruikersinterface",
    exclusive_mode_button: "Knop exclusieve modus",
    exclusive_mode_button_desc: "Toon of verberg de schakelaar voor exclusieve modus in de kopregel",
    native_rate_button: "Knop voor native samplefrequentie",
    native_rate_button_desc: "De schakelaar voor native samplefrequentie in de kop tonen of verbergen",
    repeat_shuffle: "Herhalen en willekeurig",
    repeat_shuffle_desc: "Toon of verberg de knoppen voor herhalen en willekeurig in de bediening",
    time_labels: "Tijdlabels",
    time_labels_desc: "Toon of verberg de positie en duur naast de voortgangsbalk",
    settings_now_playing: "Nu afgespeeld",
    now_playing_details: "Derde regel",
    now_playing_details_desc: "Wat er onder de titel en de artiest wordt getoond",
    now_playing_details_specs: "Formaat",
    now_playing_details_album: "Album",
    font_size: "Lettergrootte",
    font_size_desc: "Tekstgrootte in de hele app",
    font_size_small: "Klein",
    font_size_medium: "Gemiddeld",
    font_size_large: "Groot",
    blur_background: "Wazige achtergrond",
    blur_background_desc: "Gebruik de vervaagde hoes als achtergrond",
    blur_background_off: "Uit",
    blur_background_cover: "Hoesweergave",
    blur_background_all: "Alle weergaven",
    blur_intensity: "Vervagingsintensiteit",
    blur_intensity_desc: "Hoe vaag de achtergrond is",
    blur_interface_opacity: "Interface-dekking",
    blur_interface_opacity_desc: "Hoe doorzichtig de interface is boven de vervaging",
    liked_tracks: "Favorieten",
    liked_tracks_desc: "Toon het hartpictogram en het tabblad Favorieten. Uitschakelen verbergt alles.",
    playlists_desc: "Toon het tabblad Afspeellijsten en de toevoegknop op elke rij.",
    track_duration: "Duur",
    track_duration_desc: "Toon of verberg de duur in de wachtrij",
    action_buttons: "Actieknoppen",
    action_buttons_desc: "Toon of verberg de toevoeg- en favorietknoppen in de wachtrij",
    artist_name: "Artiestnaam",
    artist_name_desc: "Toon of verberg de artiestnaam in de wachtrij",
    queue_deduplication: "Duplicaten verwijderen",
    queue_deduplication_desc: "Nummers die al in de wachtrij staan overslaan bij toevoegen",
    settings_albums_view: "Albumweergave",
    albums_layout: "Indeling",
    albums_layout_desc: "Hoe albums worden getoond op het tabblad Albums",
    albums_layout_list: "Lijst",
    albums_layout_grid: "Raster",
    year_column: "Jaarkolom",
    year_column_desc: "De jaarkolom in de albumlijst tonen",
    album_year: "Jaar",
    album_year_desc: "Het jaar in het tegelbijschrift tonen",
    genre_column: "Genrekolom",
    genre_column_desc: "De genrekolom in de albumlijst tonen",
    albums_artist_desc: "Hoe de albumartiest in de lijst wordt weergegeven",
    albums_artist_inline: "Inline",
    albums_artist_column: "Kolom",
    albums_artist_hidden: "Verborgen",
    settings_artists_view: "Artiestenweergave",
    artists_group_by_tag: "Groeperen op tag",
    music_folders: "Muziekmappen",
    music_folders_desc: "Mappen die worden gescand op muziekbestanden",
    no_folders_added: "Geen mappen toegevoegd",
    remove: "Verwijderen",
    add_folder: "Map toevoegen…",
    remove_folder_confirm_title: "Map verwijderen?",

    audio_device: "Audioapparaat",
    exclusive_mode_title: "Exclusieve modus",
    bit_perfect_playback: "Bit-perfect afspelen",
    not_bit_perfect: "Niet bit-perfect:",
    bp_not_exclusive: "• Uitvoer is gedeeld (niet exclusief)",
    bp_system_muted: "• Systeem gedempt",
    bp_no_source: "• Geen bron geladen",
    select_audio_device: "Audioapparaat selecteren",
    exclusive_click_disable: "Exclusieve modus — klik om uit te schakelen",
    shared_click_enable: "Gedeelde modus — klik om exclusief in te schakelen",
    native_rate_title: "Native samplefrequentie",
    native_rate_click_disable: "Native samplefrequentie — klik om uit te schakelen",
    native_rate_click_enable: "Native samplefrequentie staat uit — klik om in te schakelen",
    bp_native_rate_off: "• Native samplefrequentie staat uit",
    default_suffix: " (standaard)",

    onboarding_title: "Welkom bij Pawse",
    onboarding_subtitle: "Laten we een paar dingen instellen voordat je gaat luisteren.",
    onboarding_theme_prompt: "Kies een kleurthema",
    onboarding_language_prompt: "Kies je taal",
    onboarding_folder_prompt: "Voeg een map met je muziek toe",
    onboarding_finish: "Beginnen met luisteren",

    quit_pawse: "Pawse afsluiten",
    rescan_library: "Bibliotheek opnieuw scannen...",
    hide_pawse: "Verberg Pawse",
    hide_others: "Verberg overige",
    show_all: "Toon alles",
    minimize: "Minimaliseren",
    zoom: "Zoom",
    menu_help: "Help",
    repository: "Repository",
    check_for_updates: "Controleren op updates...",
    automatic_updates: "Automatische updates",
    automatic_updates_desc: "Bij het opstarten automatisch op updates controleren",
    version: "Versie",
    restart_to_update: "Herstart om bij te werken",
    up_to_date: "Pawse is up-to-date",

    lyrics: "Songtekst",
    lyrics_empty: "Geen songtekst",
    lyrics_fetching: "Songtekst ophalen…",
    lyrics_not_found: "Songtekst niet gevonden",
    lyrics_save: "Opslaan in .lrc-bestand",
    lyrics_follow: "Huidige regel volgen",
    lyrics_from_internet: "Songtekst van internet ophalen",
    lyrics_from_internet_desc: "Ontbrekende songteksten online zoeken via LRCLIB. Zet uit voor privacy.",
    remote_control: "Afstandsbediening",
    lastfm_status_disconnected: "Niet verbonden.",
    lastfm_status_awaiting: "Wachten tot je pawse autoriseert in je browser…",
    lastfm_status_connected: "Verbonden",
    lastfm_sign_in: "Inloggen",
    lastfm_sign_out: "Uitloggen",
    lastfm_unavailable: "Last.fm is niet geconfigureerd in deze build.",
    settings_scrobbling: "Scrobbelen",
    scrobble_account: "Account",
    scrobble_enabled: "Ingeschakeld",
    scrobble_send_loves: "Likes versturen",
    scrobble_send_loves_desc: "Een like hier voegt het nummer ook toe aan je favorieten bij de dienst. Verwijderen wist het daar ook.",
    scrobble_file: "Bestand",
    scrobble_first_artist: "Alleen de eerste artiest scrobbelen",
    scrobble_first_artist_desc: "Eén artiestnaam per nummer versturen in plaats van alle uitvoerenden samen.",
    scrobble_csv: "CSV-logboek",
    scrobble_csv_desc: "Elke scrobble toevoegen aan een lokaal CSV-bestand.",
    scrobble_file_open: "Openen…",
    scrobble_file_new: "Nieuw…",
    scrobble_not_a_log: "Dat bestand is geen pawse-scrobblelog.",
    scrobble_token: "Token",
    scrobble_server: "Server",
    scrobble_connect: "Verbinden",
    scrobble_auth_expired: "Autorisatie verlopen, meld je opnieuw aan",
    scrobble_auth_cancel: "Annuleren",
    scrobble_auth_link_expired: "De autorisatielink is niet meer geldig. Meld je opnieuw aan voor een nieuwe.",
    scrobble_import_loves: "Favorieten importeren",
    scrobble_import_loves_desc: "Wat in je profiel gemarkeerd is, komt bij «Favorieten». Er wordt niets teruggestuurd.",
    scrobble_import_run: "Importeren",
    scrobble_import_result_t: "Gevonden in je bibliotheek: {} van {}",
    scrobble_importing: "Importeren…",
    discord_share: "Weergeven op Discord",
    discord_share_desc: "Toon het nummer dat je afspeelt op je Discord-profiel.",
    remote_control_desc: "Bedien het afspelen vanuit een webbrowser op je lokale netwerk.",
    remote_port: "Poort",
    remote_port_desc: "Netwerkpoort voor de afstandsbedieningsserver.",
    open_in_browser: "Openen in browser",
    lyrics_text_size: "Tekstgrootte",
    lyrics_text_size_desc: "Grootte van de songtekst",
    lyrics_karaoke: "Karaoke-vulling",
    lyrics_karaoke_desc: "Vul de huidige regel tijdens het afspelen",
    lyrics_dim_inactive: "Andere regels dimmen",
    lyrics_dim_inactive_desc: "Regels die niet spelen vervagen",

    disc_t: "Schijf {}",
    n_tracks_one: "{} nummer",
    n_tracks_few: "{} nummers",
    n_tracks_other: "{} nummers",
    bp_system_volume_t: "• Systeemvolume niet op maximum: {}",
    bp_sample_rate_t: "• Samplerate komt niet overeen: bron {} Hz → apparaat {} Hz",
    bp_bit_depth_t: "• Bitdiepte {} overschrijdt f32-container (24)",
    failed_exclusive_t: "Kan exclusieve modus niet inschakelen: {}",
    native_rate_failed_t: "Kan native samplefrequentie niet inschakelen: {}",
    failed_switch_device_t: "Kan niet van apparaat wisselen: {}",
    failed_save_settings_t: "Kan instellingen niet opslaan: {}",
    remote_start_failed_t: "Kan de afstandsbedieningsserver niet starten op poort {}: {}",
    update_ready_t: "Pawse {} is klaar om te installeren",
    update_check_failed_t: "Controle op updates mislukt: {}",
    cover_artist_desc: "Toon de artiest onder de titel in hoesweergave",
    cover_progress: "Voortgangsbalk",
    cover_progress_desc: "Toon een minimale voortgangsbalk in hoesweergave",
    cover_controls: "Afspeelknoppen",
    cover_controls_desc: "Toon de knoppen afspelen, volgende en vorige in hoesweergave",
    edit_tags: "Tags bewerken",
    edit_album_tags: "Albumtags bewerken",
    tag_editor: "Tag-editor (beta)",
    tag_editor_desc: "Toon bij elk nummer een knop om de tags te bewerken",
    tag_save: "Opslaan",
    tag_title: "Titel",
    tag_artists: "Artiesten",
    tag_album: "Album",
    tag_album_artists: "Albumartiesten",
    tag_track_number: "Tracknummer",
    tag_disc_number: "Schijfnummer",
    tag_year: "Jaar",
    tag_genres: "Genres",
    tag_add_genre: "Genre toevoegen",
    tag_all_tags: "Alle tags in het bestand",
    tag_custom_key: "Tagnaam",
    tag_custom_value: "Waarde",
    tag_custom_invalid: "Deze tagnaam kan niet worden gebruikt: hij is ongeldig of staat al bij de velden hierboven.",
    tag_custom_invalid_len: "ID3v2 kan geen tagnaam van 4 tekens opslaan — dat is de lengte van een frame-id. Kies een langere.",
    tag_cover: "Hoes",
    tag_cover_replace: "Vervangen…",
    tag_cover_remove: "Verwijderen",
    tag_cover_will_remove: "De hoes wordt bij opslaan verwijderd.",
    tag_cover_album_hint: "Wordt in elk bestand van het album geschreven.",
    tag_cover_failed_title: "Kan die afbeelding niet gebruiken",
    tag_cover_unreadable: "Dit bestand kon niet worden gelezen.",
    tag_cover_bad_format: "Een hoes moet een JPEG of een PNG zijn.",
    tag_cue_readonly: "Dit nummer komt uit een CUE-sheet. De tags staan in het .cue-bestand en zijn hier niet te bewerken.",
    tag_playing_readonly: "Dit is het huidige nummer. Opslaan zou het bestand overschrijven dat de speler open heeft — schakel eerst naar een ander nummer.",
    tag_album_playing_readonly: "Het huidige nummer hoort bij dit album. Opslaan zou het bestand overschrijven dat de speler open heeft — schakel eerst naar een ander nummer.",
    tag_album_fields_hint: "Albumvelden gelden voor het hele album — bewerk ze in de album-tag-editor.",
    tags_saved: "Tags opgeslagen",
    tag_write_failed_title: "Tags schrijven mislukt",
    tags_save_failed_t: "Tags schrijven mislukt: {}",
};
